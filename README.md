...
这是一个 Tampermonkey / 用户脚本，用于自动化预约深圳大学（SZU）体育场馆。脚本兼容 iOS、Android、iPad、移动端与桌面端，提供智能预约、定时预约、重试/限频策略与可选的企业微信通知。
...
1. 在“定时预约”区域选择目标日期和触发时间
2. 设置定时并保存，脚本将在到点自动开始预约
3. 页面刷新后定时任务会自动恢复（因为配置已持久化）

桌面快捷键（仅桌面端）：

- Ctrl + Shift + S：开始 / 停止预约
